// src/pages/ai/PlateRecognitionPage.jsx
import React, { useState, useRef, useContext } from 'react';
import { Camera, Upload, RefreshCw, Save, CheckCircle, AlertCircle, X } from 'lucide-react';
...
const PlateRecognitionPage = () => {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      console.error('Error in license plate detection:', err);
      error(err.response?.data?.detail || 'خطا در تشخیص پلاک');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <PageHeader
        ...
        subtitle="با استفاده از هوش مصنوعی، پلاک خودرو را به صورت خودکار تشخیص دهید"
        ...
        <Card title="نتیجه تشخیص">
          {result ? (
                ...
                <>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gray-100 p-4 rounded-lg">
                      <h3 className="text-lg font-bold text-center mb-2">{result.license_plate}</h3>
                    ...
                    <div className="bg-gray-100 p-4 rounded-lg">
                      <h4 className="font-medium mb-2">اطلاعات موقعیت:</h4>
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>X1: {result.bbox[0]}</div>
                        <div>Y1: {result.bbox[1]}</div>
                        <div>X2: {result.bbox[2]}</div>
                        <div>Y2: {result.bbox[3]}</div>
                      </div>
                    </div>
            ...
            <div className="flex flex-col items-center justify-center p-10 text-center">
              <p className="text-gray-500 mb-2">منتظر تشخیص پلاک...</p>
              <p className="text-sm text-gray-400">لطفاً تصویری از پلاک خودرو انتخاب کنید و دکمه تشخیص را بفشارید</p>
            </div>
          )}
        </Card>
      </div>
